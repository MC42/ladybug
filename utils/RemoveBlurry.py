...
def evalBlur(folder, AcceptableBlur=200):
    ...
    for folder in rawFolders:
        ...
        imagePaths = [
            ...
            if isfile(os.path.join(folder, file))
        ]
        ...
        for imagePath in imagePaths:
            ...
            if (
                len(FolderBadImages) == len(imagePaths) and len(FolderBadImages) > 0
            ):  # all fail metric, keep best
...
if __name__ == "__main__":
    ...
    if args["extension"]:
        ...
        main(args["input"], blur, extension=extension)
    else:
