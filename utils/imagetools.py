...
def image_quarter(img, number):
    ...
    if number == 0:
        ...
        return img[x_len // 2 : x_len - x_len % 2, 0 : y_len // 2, :]
    elif number == 2:
        return img[0 : x_len // 2, y_len // 2 : y_len - y_len % 2, :]
    else:
        return img[x_len // 2 : x_len - x_len % 2, y_len // 2 : y_len - y_len % 2, :]
...
def reconstruct_max_3d(list_iterators, final_dims, key=calculate_sharp):
    ...
    prev_images, depth_index = max_images(
        list_sub_images, key=calculate_sharp, dimensional=True
    )
    prev_depth_images = np.full(
        (prev_images.shape[0], prev_images.shape[1]), depth_index
    )
    for y in range(1, final_dims[1], 1):
        list_sub_images = [next(image_gen) for image_gen in list_iterators]
        best_image, depth_index = max_images(
            list_sub_images, key=calculate_sharp, dimensional=True
        )
        best_depth_image = np.full(
            (best_image.shape[0], best_image.shape[1]), depth_index
        )
        prev_images = np.concatenate((prev_images, best_image), axis=1)
        prev_depth_images = np.concatenate(
            (prev_depth_images, best_depth_image), axis=1
        )
...
def max_pool_subdivided_images(images, subdiv_dims=(4, 4)):
    ...
    subimage_generators = [
        partition_image(image, (image.shape[0] // subdiv_x, image.shape[1] // subdiv_y))
        for image in images
    ]
...
def max_pool_subdivided_images_3d(images, subdiv_dims=(4, 4)):
    ...
    (best_image, IndexMap) = reconstruct_max_3d(
        subimage_generators, (subdiv_x, subdiv_y)
    )
...
def frame_hue_bounded(
    frame, lower_bound=np.array([20, 100, 100]), upper_bound=np.array([30, 255, 255])
):
...
def overlay(canvas, sub_img, location):
    ...
    canvas[
        location[0] : location[0] + ys_len, location[1] : location[1] + xs_len
    ] = sub_img
...
def GenerateCanvas(AbsoluteWidth=10, AbsoluteHeight=10, PixelsPerUnit=100):
    ...
    WidthInPixels = (
        int(math.ceil(AbsoluteWidth * PixelsPerUnit / 10.0)) * 10
