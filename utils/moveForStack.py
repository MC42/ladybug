...
if loc_of_of != -1 and require_input == False:  # we should be using our naming scheme:
    ...
    if num_rotations.isdigit():
        ...
        num_rotations = int(
            input(
                "I dont get the naming scheme. Please enter how many rotations there are"
            )
        )
else:
    num_rotations = int(
        input("I dont get the naming scheme. Please enter how many rotations there are")
    )
...
if not stackable_files.is_integer():
    ...
    stackable_files = int(
        input(
            "Files/rotations is not a whole number. Please check that you have no extraneous or missing files. Looking for type {}. You can try typing in the number of unique stackable files".format(
                filetype
            )
        )
    )
