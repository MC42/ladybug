...
def crop(
    img, yDim, xDim, side
):  # crops image evenly on all sides to specified dimensions, or from specific side (evenly on other axis)
...
def scanDim(folder, extension):
    ...
    for file in os.listdir(folder):
        if file.endswith(extension):
            ...
            )  # inefficient to actually read the image each time. We just care about resolution. Alternative?
            height, width, channels = tempImg.shape
            minH = (
                height if height < minH else minH
            )  # the file's height becomes minh if smaller than current height. else necessary?
    ...
    for currentDir in rawFolders:
        ...
        for file in imgList:
            ...
            if side != "smart":
                ...
                print(
                    "we commented out smartcrop. So I guess go edit the file and un-uncomment it"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--side",
        required=False,
        help="side to preferentially crop from. left, right, top, bottom, smart (attempts to find center of image and crop from that, currently commented out), all. defaults to all",
    )
