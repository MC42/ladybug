...
def ConvertXYToPixelLocations(
    X=-1, Y=-1, PixelsPerMM=370, ImageXWidth=640, ImageYHeight=480
):
...
def GoToSmush(
    ...
    XLow, XHigh, YLow, YHigh = ConvertXYToPixelLocations(
        X=X,
        Y=Y,
        PixelsPerMM=PixelsPerMM,
        ImageXWidth=ImageXWidth,
...
def StackFolder(folder, StackedOutputFolder, ZMapOutputFolder, grid=(32, 32)):
    ...
    for i, folder in enumerate(ImageFolders):
        ...
        DepthImage = NormalizeZMap(
            TrueZ3D, lowest_Z=lowest_Z, highest_Z=highest_Z
...
def StitchFolder(folder, PixelsPerMM=370, GiantSize="default", StitchZMap=False):
    ...
    if not StitchZMap:
        MajorImage = MakeGiantImage(
            PixelsPerMM=PixelsPerMM,
            CanvasXMin=0,
            CanvasXMax=GiantSize,
            CanvasYMin=0,
            CanvasYMax=GiantSize,
        )
    else:
        MajorImage = MakeGiantImage(
            PixelsPerMM=PixelsPerMM,  # ZMap image! FLOATS!
            CanvasXMin=0,
            CanvasXMax=GiantSize,
            CanvasYMin=0,
            CanvasYMax=GiantSize,
...
def MakeGiantImage(
    ...
    if dim == 1:  # quick fix for pure 2d matrices. Sorry!
        ...
        if floats:  # too tired to save 2 lines with proper fix
            ...
            YujieImage = np.zeros(
                (YPixels, XPixels, dim), np.uint8
            )  # square image because plate is square, surprise!
    ...
    for i in range(tries):  # communication back and forth not instant, i for failsafe
        ...
        if "Count" in dump:  # precedes actual position data
            try:
                ...
                X = float(
                    "".join([s for s in Xraw if (s.isdigit() or s == ".")]).strip()
                )
                Y = float(
                    "".join([s for s in Yraw if (s.isdigit() or s == ".")]).strip()
                )
                Z = float(
                    "".join([s for s in Zraw if (s.isdigit() or s == ".")]).strip()
                )
...
def TryToConnect(port, BAUD, timeout, return_anyway=False):
    ...
    print("trying to connect with port {} and BAUD {}...".format(port, BAUD))
...
def AutoCoin(
    ...
    if positions["X"] == 0 or positions["Y"] == 0 or positions["Z"] == 0:

        Home()

    WaitForConfirmMovements(0, 0, 0)
    TrueCoins = {
        (0, 0): {"R": 0, "PointsOfFocus": []}
    }  # (X, Y): Radius, FocusHeights, anything else
    ...
    for i in range(len(XLocations)):  # 1: rough grid search
        ...
        XYFocusPoints = DivideCircle(
            XMiddle, YMiddle, Radius, FocusPoints
        )  # optional: scanlocations
        ...
        for point in XYFocusPoints:  # places to check focus
            ...
            FocusHeight, CoinFocusPic = FindZFocus(
                ZHeights, Comprehensive=True
            )  # Future: analyze subimage
            ...
            TrueCoins[(XMiddle, YMiddle)]["PointsOfFocus"].append(
                (xfocus, yfocus, FocusHeight, CoinFocusPic)
            )
            FocusSet.add(
                FocusHeight
            )  # Future: Make sure it's not too many images, or use best ones
...
def FindMissingLocations(FullLocations, PartialLocations):
    ...
    for i in range(len(PartialLocations["X"])):
        ...
        PartX, PartY, PartZ, PartR = (
            PartialLocations["X"][i],
...
def SortOrStackPipe(
    ...
    if not FocusDictionary:  # not passed in, try to import pickled file
        try:
            ...
            print(
                "unable to find pickle focus dictionary from previous scan. Generating now..."
            )
            FocusDictionary = {}
...
def GridToCircle(GridLocations, XCenter, YCenter, Radius):
    ...
    print(
        "{} images reduced to {} upon circularification".format(
            len(ScanLocations["X"]) + len(track), len(ScanLocations["X"])
        )
    )
...
def CalculateOutline(
    ...
    if shape == "coin":
        ...
        if YUpper and YLower:
            Radius, YMiddle = round((YUpper - YLower) / 2, 1), round(
                (YLower + YUpper) / 2, 1
            )
        else:
...
def DefineScan(
    ...
    ScanLocations = {
        "X": [round(num, 2) for num in NewNewXScan],
...
def RotateScan(ScanLocations, degrees=30):
    ...
    ZLocations = [
        round((x - XOffset) * sinof + z, 2) for x, z in zip(XLocations, ZLocations)
    ]
...
def StartCamera(camera=0, Width=640, Height=480):
    ...
    (
        ret,
        junkframe,
    ) = cap.read()  # junk because must grab first frame THEN set LED controls
...
def ShowCamera(
    ...
    ColorLowers = [
        (64, 255, 255),
        (10, 100, 20),
    ]  # note lower"s". cycle through to get color
    ...
    StackedPics = [
        ...
        for fn in next(os.walk(StackPath))[2]
        if ".jpg" in fn
    ]
    ...
    while True:
        ...
        if SCount % 50:
            ...
            NewPics = list(
                list(set(StackedPics) - set(TempStacked))
                + list(set(TempStacked) - set(StackedPics))
            )
            if NewPics:
        ...
        if k % 256 == ord("c"):  # toggle color choice
            ...
            ColorLower, ColorUpper = (
                ColorLowers[ColorsIndex],
                ColorUppers[ColorsIndex],
            )  # brown
            print(
                "colors track toggled. lower hsv range: {} upper {}".format(
                    ColorLower, ColorUpper
                )
            )
        ...
        if k % 256 == ord("s"):
            ...
            initBB = cv2.selectROI(
                DefaultName, frame, fromCenter=False, showCrosshair=True
            )
        ...
        if initBB is not None:
            # call bounding boxer
            frame, tracker, fps, success, box = ObjectTracker.UpdateBox(
                frame, tracker, fps
            )
            if not success:
                BoxTimeout += 1
        ...
        if TrackTheBug:  # trackthebug by color needs consolidation with box method
            ...
            TrackerOutput = ObjectTracker.BallTracker(
                frame,
                ColorLower=ColorLower,
                ColorUpper=ColorUpper,
                Width=Width,
                Height=Height,
            )  # False if nothing detected
...
def KeepBugInCenter(
    ...
    if YMovement or XMovement:
        ...
        AllGoTo(
            XFinal, YFinal, GlobalZ, update=False, speed=speed
        )  # not waiting for update yet

    return XFinal, YFinal


def DefaultStack(pics=20, stepsize=0.1, dims=(32, 32), center="default"):
    ...
    stacked, allframes, TrueZ3D = ZStackKinda(
        ZCoord, subdiv_dims=dims
    )  # the juicy part

    DepthMap = NormalizeZMap(TrueZ3D)

    name = "Capture\\Stacked\\{} to {} at X {}, Y {}.jpg".format(
        low, high, GlobalX, GlobalY
    )
    DepthName = "Capture\\Stacked\\depth {} to {} at X {}, Y {}.jpg".format(
        low, high, GlobalX, GlobalY
    )
...
def ZStackKinda(ZCoord, subdiv_dims=(4, 4), camera="default", X=-1, Y=-1):
...
def GetTrueZ3D(IndexMap, ZCoord):
    ...
    indexer = np.array(
        [d.get(i, -1) for i in range(IndexMap.min(), IndexMap.max() + 1)]
    )
...
def CallibratePlate(
    CoordinatePoints=[(25, 60), (90, 125), (105, 30), (62, 75)],
    LowPoint=3,
    HighPoint=5,
    Precision=0.1,
    ShowImage=False,
    ...
    for count, point in enumerate(CoordinatePoints):
        ...
        print(
            "Focus for Point {} (X:{},Y{}) at Z Height = {}".format(
...
def FindZFocus(
    ...
    if ZCoord == "broad":
        ZCoord = GenerateZ(
            (GlobalZ - 10 if GlobalZ - 10 >= 0 else 0), GlobalZ + 10, 0.5
        )
        ScannedBroad = True  # scan again with narrower range

        if GlobalZ >= 0.5:
            AllGoTo(
                GlobalX, GlobalY, GlobalZ - 0.5, update=False
            )  # if you're already focused this is faster

    elif ZCoord == "narrow":
        ZCoord = GenerateZ(
            (GlobalZ - 0.6 if GlobalZ - 0.6 >= 0 else 0), GlobalZ + 0.6, 0.1
        )
        if GlobalZ >= 0.3:
    ...
    for i, Z in enumerate(ZCoord):
        ...
        if GoingUp:
            ...
            blurs.append(blur)
            if (
                Comprehensive == False and i >= 2
            ):  # arrest search if overshoot focus point
                if (
                    (blurs[i - 2] > 50)
                    and (blurs[i] < blurs[i - 1])
                    and (blurs[i - 1] < blurs[i - 2])
                ):
                    break
            ...
            TrueMax = blurs.index(
                max(blurs)
            )  # (more spaghetti) THIS WAS ONE LINE DOWN UNDER IF COMPREHENSIVE
            if (
                Comprehensive == False and i > 2
            ):  # arrest search if overshoot focus point
                ...
                if TrueMax > 2:  # going down. don't worry about absolutes
                    if (
                        (blurs[TrueMax] > 50)
                        and (blurs[TrueMax] > blurs[TrueMax - 1])
                        and (blurs[TrueMax - 1] > blurs[TrueMax - 2])
                    ):
                        break
...
def MakeFolderFromPositions(X, Y, Z, R, ParentFolder, FileType=".jpg"):
    ...
    if not os.path.exists(
        folder
    ):  # should hopefully continue saving in the same folder after restart
        os.makedirs(folder)
...
def GridScan(ScanConditions):  # DefaultScan dictionary available for modifying
    ...
    for i in range(
    ...
    ):  # pointinscan 0 default, invoked in restart
        ...
        SuccessfulWait = WaitForConfirmMovements(
            X, Y, (Z if not AutoFocus else GlobalZ)
        )
        ...
        if not SuccessfulWait:
            ...
            SuccessfulWait = WaitForConfirmMovements(
                X, Y, (Z if not AutoFocus else GlobalZ), attempts=100
            )
        ...
        for i in range(3):  # catches some failed pictures
            ...
            try:
                ...
                if PointInScan % 100 == 0:  # every 100 pics
                    ...
                    if len(ScanRates) > 1:
                        tttt = int(
                            ScanRates[-1] - ScanRates[-2]
                        )  # time taken this time
                        print(
                            "{} of {} pics took {} seconds".format(
                                PointInScan, len(XCoord), tttt
                            )
                        )
...
def AllGoTo(
    ...
    SendGCode(GCode)
    GlobalX, GlobalY, GlobalZ, GlobalE = (
        round(X, 2),
        round(Y, 2),
        round(Z, 2),
        round(E, 2),
...
def InitiatePCBTools():
    ...
    while True:
        ...
        choice = input(
            "enter component name to goto and snap picture or 'all' to do all. Control C to exit"
        )
...
def MoveXLeftBig():  # dir distance delay
    (XGoTo(GlobalX - BigXY), print("X decreased by", BigXY)) if (
        GlobalX - BigXY
    ) > XMin else (XGoTo(XMin), print("X at Min ({})".format(XMin)))


def MoveXLeftSmall():
    (XGoTo(GlobalX - LittleXY), print("X decreased by", LittleXY)) if (
        GlobalX - LittleXY
    ) > XMin else (XGoTo(XMin), print("X at Min ({})".format(XMin)))


def MoveXRightBig():
    (XGoTo(GlobalX + BigXY), print("X increased by", BigXY)) if (
        GlobalX + BigXY
    ) < XMax else (XGoTo(XMax), print("X at Max ({})".format(XMax)))


def MoveXRightSmall():
    (XGoTo(GlobalX + LittleXY), print("X increased by", LittleXY)) if (
        GlobalX + LittleXY
    ) < XMax else (XGoTo(XMax), print("X at Max ({})".format(XMax)))


def MoveYForwardBig():
    (YGoTo(GlobalY + BigXY), print("Y increased by", BigXY)) if (
        GlobalY + BigXY
    ) < YMax else (YGoTo(YMax), print("Y at Max ({})".format(YMax)))


def MoveYForwardSmall():
    (YGoTo(GlobalY + LittleXY), print("Y increased by", LittleXY)) if (
        GlobalY + LittleXY
    ) < YMax else (YGoTo(YMax), print("Y at Max ({})".format(YMax)))


def MoveYBackBig():
    (YGoTo(GlobalY - BigXY), print("Y decreased by", BigXY)) if (
        GlobalY - BigXY
    ) > YMin else (YGoTo(YMin), print("Y at Min ({})".format(YMin)))


def MoveYBackSmall():
    (YGoTo(GlobalY - LittleXY), print("Y decreased by", LittleXY)) if (
        GlobalY - LittleXY
    ) > YMin else (YGoTo(YMin), print("Y at Min ({})".format(YMin)))


def MoveZDownBig():  # dir dis delay
    (ZGoTo(GlobalZ - BigZ), print("Z decreased by", BigZ)) if (
        GlobalZ - BigZ
    ) > ZMin else (ZGoTo(ZMin), print("Z at Min ({})".format(ZMin)))


def MoveZDownSmall():
    (ZGoTo(GlobalZ - LittleZ), print("Z decreased by", LittleZ)) if (
        GlobalZ - LittleZ
    ) > ZMin else (ZGoTo(ZMin), print("Z at Min ({})".format(ZMin)))


def MoveZUpBig():
    (ZGoTo(GlobalZ + BigZ), print("Z increased by", BigZ)) if (
        GlobalZ + BigZ
...
YPosition = tk.Label(
    TopFrame, font=(myFont), height=2, width=12
)  # use a Label widget, not Text
...
YForwardBigButton = tk.Button(
    TopFrame, text="⇑", font=myFont, command=MoveYForwardBig, height=1, width=2
)
...
YBackSmallButton = tk.Button(
    TopFrame, text="↓", font=myFont, command=MoveYBackSmall, height=1, width=2
)
YBackSmallButton.pack(side=tk.TOP)

YBackBigButton = tk.Button(
    TopFrame, text="⇓", font=myFont, command=MoveYBackBig, height=1, width=2
)
...
XPosition = tk.Label(
    LeftFrame, font=(myFont), height=2, width=12
)  # use a Label widget, not Text
...
XLeftBigButton = tk.Button(
    LeftFrame, text="⟸", font=myFont, command=MoveXLeftBig, height=1, width=2
)
XLeftBigButton.pack(side=tk.LEFT)

XLeftSmallButton = tk.Button(
    LeftFrame, text="←", font=myFont, command=MoveXLeftSmall, height=1, width=2
)
...
XRightBigButton = tk.Button(
    LeftFrame, text="⟹", font=myFont, command=MoveXRightBig, height=1, width=2
)
XRightBigButton.pack(side=tk.LEFT)

ZPosition = tk.Label(
    RightFrame, font=(myFont), height=2, width=12
)  # use a Label widget, not Text
...
ZUpBigButton = tk.Button(
    RightFrame, text="Z⇑", font=myFont, command=MoveZUpBig, height=1, width=2
)
ZUpBigButton.pack(side=tk.RIGHT)

ZUpSmallButton = tk.Button(
    RightFrame, text="Z↑", font=myFont, command=MoveZUpSmall, height=1, width=2
)
...
ZDownBigButton = tk.Button(
    RightFrame, text="Z⇓", font=myFont, command=MoveZDownBig, height=1, width=2
)
ZDownBigButton.pack(side=tk.RIGHT)

HomeXButton = tk.Button(
    BottomFrame, text="HOME X", font=myFont, command=HomeX, height=2, width=8
)
HomeXButton.pack(side=tk.BOTTOM, pady=5)

HomeYButton = tk.Button(
    BottomFrame, text="HOME Y", font=myFont, command=HomeY, height=2, width=8
)
HomeYButton.pack(side=tk.BOTTOM, pady=5)

HomeZButton = tk.Button(
    BottomFrame, text="HOME Z", font=myFont, command=HomeZ, height=2, width=8
)
HomeZButton.pack(side=tk.BOTTOM, pady=5)

AutoCoinButton = tk.Button(
    BottomFrame,
    text="AUTOCOIN",
    font=mySmallFont,
    command=StartAutoCoin,
    height=1,
    width=15,
)
