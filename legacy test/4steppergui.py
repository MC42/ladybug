...
YScanStep = (
    150  # reasonable default for field of view. Can scretch to 150 for faster scans.
)
...
FactorsOf160 = [
    ...
    80,
    160,
]  # for drop down menu of rotations of R since 20 step motor with 8th micro
...
GPIO.setup(
    ZLimit, GPIO.IN, pull_up_down=GPIO.PUD_UP
)  # no pull up! Direct sense with optical switch.
...
def MoveY(direction, numsteps, delay):
    ...
    if direction == YFORWARD:
        ...
        GlobalY -= numsteps
    YPosition.configure(
        text="Y: " + str(GlobalY) + "/" + str(YMax)
    )  # REMOVE IF CONVERTING TO HEADLESS WITHOUT GUI
...
def RGoTo(RDest, RMin=0):
    ...
    if RDest <= StepsPerRotation and RDest >= RMin:
        ...
        print(
            "I understand the desire to watch the motor spin around a lot... but between {} and {} please".format(
                RMin, StepsPerRotation
            )
        )
...
def HomeX():
    ...
    for i in range(
        XMax + 200
    ):  # some number that's noticably larger than the range, but also will eventually stop in case something goes wrong

        # check if button is pressed

        if CheckPress(
            XLimit
        ):  # button pressed once. need to move forward and back again to ensure correct start position
            ...
            for j in range(350):  # move back and check again
                if CheckPress(XLimit):  # again
                    ...
                    print(
                        "was already homed check: took {} out of 300 steps on the second bounce".format(
                            j
                        )
                    )
                    GlobalX = 0
        ...
        if CheckPress(
            ...
            for j in range(1400):  # move back and check again
                if CheckPress(ZLimit):  # again
                    ...
                    MoveZ(
                        ZBACKWARD, 1000, FAST
                    )  # START AT MINIMUM RANGE for easier calculating

                    print("Optical switch has been tripped after {} steps!".format(i))
                    print(
                        "was already homed check: took {} out of 1200 steps on the second bounce".format(
                            j
                        )
                    )
...
def DefineScan(
    ...
    XMax = (
        XMax + 1
    )  # its not inclusive in max and will break if min = max, so we add 1 to all maximums.
    YMax = YMax + 1
    ZMax = ZMax + 1
    RMax = RMax + 1
...
def GridScan(ScanLocations, conditions="default"):
    ...
    if conditions == "default":  # usually the case!
        ...
        )  # pop up screen asking where to save files like flash drive
        filetype = ".jpg"  # jpgs are honestly ok too and might make things easier/faster #NOTE CHANGED FROM PNG DEC 16 2019
        resolution = (
            "640x480"  # for crappy microscope, not necessarily reliable, though
        )
    ...
    for i in range(num_pictures):
        ...
        if i % 100 == 0:  # Should be percentage of remaining but this is ok
            print(
                "{} of {} pictures remaining".format((num_pictures - i), original_pics)
            )
            GPIO.output(BEEP, GPIO.HIGH)
        ...
        folder = (
            save_location
            + "/Z"
            + str(ZCoord[i]).zfill(4)
            + "R"
            + str(RCoord[i]).zfill(3)
        )
        ...
        name = (
            "X"
            + str(XCoord[i]).zfill(4)
            + "Y"
            + str(YCoord[i]).zfill(4)
            + "Z"
            + str(ZCoord[i]).zfill(4)
            + "R"
            + str(RCoord[i]).zfill(3)
            + "of"
            + str(NumberOfRotations).zfill(3)
            + filetype
        )
        ...
        try:  # Largely, problems are due to USB disconnecting or just not being in.
            for w in range(3):  # try to take pic with fswebcam
                ...
                )  # like check_call(infinite timeout)
                ...
                if os.path.isfile(folder + "/" + name):  # check if file was created
                    ...
                    print(
                        "HEY BOZO THE USB GOT UNPLUGGED UNPLUG IT AND PLUG IT BACK IN WITHIN {} SECONDS OR WE REBOOT".format(
                            timeallowed
                        )
                    )
                    ...
                    time.sleep(
                        timeallowed
                    )  # Sorry, it will beep the full time, even if you restart it immediately
                    ...
                    }  # after restart because no gui timeout after 0 seconds
                    ...
                    scan_file = open(
                        "/home/pi/Desktop/ladybug/scandata.pkl", "wb"
                    )  # SCAN DATA LOCATION HARDCODED ONTO DESKTOP
    ...
    print(
        "scan completed successfully after {} seconds! {} images taken and {} restarts".format(
    ...
    try:
        os.rename(
            "/home/pi/Desktop/ladybug/scandata.pkl",
    ...
    )  # hang to await confirmation scan completed if run in autoclosing shell
...
def XRepeatTest(num_trials=100):
    ...
    print(
        "after {} total steps, over {} movements, it took {} steps to home instead of {}, for an imperfection of {}".format(
...
def YRepeatTest(num_trials=100):
    ...
    for i in range(num_trials):
        ranloc = random.randrange(
            100, YMax - 100
        )  # more than 0, less than max, in case misteps bring it to end of range
        total_steps += abs(ranlocold - ranloc)
        ranlocold = ranloc
        ListOfLoc.append(ranloc)
...
def MultiRepeatTest(num_repeats=100):
    ...
    for i in range(num_repeats):  # total trials
        ...
        XHomeData.append(
            numpertrial
        )  # aka, final is [total steps, imperfection, num_times it moved, ListOfLoc]
...
def GuiScan():
    ...
    RScanStep = (
        160 / RScanNumber
    )  # Temporary conversion from number of times to move R, with amount of steps moved each time R is moved.
    print(RScanStep)

    CallForGrid = DefineScan(
        XScanMin,
        XScanMax,
        YScanMin,
        YScanMax,
...
XStepButton = tk.Button(
    LeftFrame,
    text="Set XScan Stepsize ",
    font=myFont,
    command=SetXStep,
    height=1,
    width=20,
...
XLowerBoundButton = tk.Button(
    LeftFrame,
    text="Set XScan Min",
    font=myFont,
    command=SetXLowerBound,
    height=1,
    width=20,
)
XLowerBoundButton.pack(side=tk.TOP, pady=5)
XUpperBoundButton = tk.Button(
    LeftFrame,
    text="Set XScan Max",
    font=myFont,
    command=SetXUpperBound,
    height=1,
    width=20,
)
XUpperBoundButton.pack(side=tk.TOP, pady=5)

YStepButton = tk.Button(
    TopFrame,
    text="Set YScan Stepsize ",
    font=myFont,
    command=SetYStep,
    height=1,
    width=20,
...
YUpperBoundButton = tk.Button(
    TopFrame,
    text="Set YScan Max ",
    font=myFont,
    command=SetYUpperBound,
    height=1,
    width=20,
)
YUpperBoundButton.pack(side=tk.BOTTOM, pady=5)
YLowerBoundButton = tk.Button(
    TopFrame,
    text="Set YScan Min ",
    font=myFont,
    command=SetYLowerBound,
    height=1,
    width=20,
)
...
ZStepButton = tk.Button(
    RightFrame,
    text="Set ZScan Stepsize ",
    font=myFont,
    command=SetZStep,
    height=1,
    width=20,
...
ZLowerBoundButton = tk.Button(
    RightFrame,
    text="Set ZScan Min ",
    font=myFont,
    command=SetZLowerBound,
    height=1,
    width=20,
)
ZLowerBoundButton.pack(side=tk.TOP, pady=5)
ZUpperBoundButton = tk.Button(
    RightFrame,
    text="Set ZScan Max ",
    font=myFont,
    command=SetZUpperBound,
    height=1,
    width=20,
)
...
RCWSmallButton = tk.Button(
    BottomFrame, text="↻", font=myBigFont, command=MoveRCWSmall, height=1, width=2
)
RCWSmallButton.pack(side=tk.BOTTOM, pady=5)

RCCWSmallButton = tk.Button(
    BottomFrame, text="↺", font=myBigFont, command=MoveRCCWSmall, height=1, width=2
)
RCCWSmallButton.pack(side=tk.BOTTOM, pady=5)

ScanButton = tk.Button(
    BottomFrame, text="SCAN!!!", font=myBigFont, command=GuiScan, height=1, width=20
)
...
RSetButton = tk.Button(
    SecondaryBottomFrame,
    text="Set number of rotations",
    font=myFont,
    command=SetR,
    height=1,
    width=20,
...
keypress_button = tk.Checkbutton(
    SecondaryBottomFrame,
    text="ENABLE KEYBOARD CONTROLS",
    variable=keypress_var,
    command=allow_keypress,
)
keypress_button.pack(side=tk.RIGHT)


LeftLabel = tk.Label(win)
LeftLabel._repeat_freq = int(
    SLOW * 1000 * 10
)  # holding Down key, milisecond per repeat.. Delay should be how long it actually takes to move
LeftLabel._repeat_on = True
...
try:
    ...
    conditions = scan_params[
        1
    ]  # save location, filetype, resolution, timeout, numfailures

    GlobalR = conditions[
        "R_Location"
    ]  # I'm worried that pins would flip here during restart. Likely a source of error
