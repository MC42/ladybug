...
ZScanStep = 500
RScanNumber = (
    1  # needs consolidation with absolute value positioning system. kind of a mess
)
...
FactorsOf160 = [
    ...
    80,
    160,
]  # for drop down menu of rotations of R
...
def TiltCorrection(Initial, Final, Radius="dynamic", X="default", Z="default"):
    ...
    if (
        X == "default"
    ):  # cannot assign globalX in function name or it happens at runtime
        X = GlobalX
    if Z == "default":
        Z = GlobalZ
    ...
    try:
        ...
        X_Change_Steps = -1 * round(
            XStepsPerMM * X_Change
        )  # negative because starting from the left
...
def AnalogGet(
    samples=1, Gain=GAIN
):  # returns differential analog output from photodiode
    Analog_List = []
    for i in range(
        samples
    ):  # maybe add hardcode delay. Helps variance problems, can be made to report standard deviation
...
def FindLaserFocus(ZMin=0, ZMax=3000, StepSize=1, Sample_Number=5, GoToFocus=True):
    ...
    AnalogPeak = max(
        AnalogList
    )  # note that if you actually look at the data, there tends to be 20 microns in each direction of ogood signaol
...
def CalculateOverlap(XSteps, YSteps, PixelsPerStep=1, XWidth=640, YHeight=480):
    ...
    print(
        "X overlap is {}%, Y Overlap is {} if {} Pixels Per Step displacement".format(
            XOverlap, YOverlap, PixelsPerStep
        )
    )
...
def LaserScan(ScanLocations, Adjust_Z=True, Sample_Number=5, Z_Tolerance=20):
    ...
    if not Adjust_Z:
        ...
        Analog_Peak, Z_Peak = FindLaserFocus(
            2000, 3000, 5
        )  # skips each 2 to just get in right area. #switch to 0-3k after testing
    # RGoTo(int(RCoord[0]))
    ...
    for i in range(num_pictures):
        ...
        if not Adjust_Z:
            ...
            DigitalList.append(
                AnalogGet(Sample_Number)
            )  # differential output comparing PD voltage source to PD output
        else:
            Analog_Peak, Z_Peak = FindLaserFocus(
                (Z_Peak - Z_Tolerance), (Z_Peak + Z_Tolerance), 1, Sample_Number
            )  ##note might fail if peak around max end of Z range
    ...
    ScanResults = [
        ...
        [YCoord],
        [DigitalList],
    ]  # just to simplify handling. dump to pickle
...
def GridScan(ScanLocations, conditions="default"):
    ...
    for i in range(num_pictures):
        ...
        name = (
            "X"
            + str(XCoord[i]).zfill(5)
            + "Y"
            + str(YCoord[i]).zfill(5)
            + "Z"
            + str(ZCoord[i]).zfill(5)
            + "R"
            + str(RCoord[i]).zfill(4)
            + filetype
        )
        ...
        try:
            for w in range(3):
                ...
                if os.path.isfile(
                    folder + "/" + name
                ):  # better than checking time elapsed...
    ...
    try:
        ...
        )  # quick fix to avoid infinite loop while still being able to analyze
...
def MoveX(direction, numsteps, delay):
    ...
    XPosition.configure(
        text="X: " + str(GlobalX) + "/" + str(XMax)
    )  # updates the global position on the screen. Not a good way to do it!
...
TUpSmallButton = tk.Button(
    BottomFrame, text="TUP", font=myBigFont, command=MoveTUpSmall, height=1, width=2
)
TUpSmallButton.pack(side=tk.BOTTOM, pady=5)

TDownSmallButton = tk.Button(
    BottomFrame, text="TDN", font=myBigFont, command=MoveTDownSmall, height=1, width=2
)
