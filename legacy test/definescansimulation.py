...
def GridScan(ScanLocations, conditions="default"):
    ...
    for i in range(num_pictures):
        ...
        if i != 0:
            if (
                ZCoord[i] != ZCoord[i + 1]
            ):  # new Z height. Make a new folder and start saving
        ...
        for i in range(
            2
        ):  # I am wrapping the whole thing in a true loop to check if proc completed too fast (no usb), then waiting for user to reboot usb.
            try:
    ...
    # QuitCamera()
    print(
        "scan completed successfully after {} seconds! {} images taken".format(
